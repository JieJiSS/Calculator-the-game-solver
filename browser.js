/* Script by JieJiSS, in France & Italy, 2017. Presented with ❤
 * @require environment: modern browser (with `console`)
 * @usage run `main()` and follow the instruction.
 * @debug run `show_re()` to view all regexps.
...
function main() { // Script By JieJiSS
    var answer = [],
        vals = [];
    try {
        var init = Number(prompt('Initial value:')),
            goal = Number(prompt('Target value:')),
            move = Number(prompt('Number of steps:'));
        var strarr = prompt('Texts on buttons: (use space to seperate)').split(/[\s\,]+/);
        var door = prompt('Portal (optional):');
        var positions;
...
function show_re() {
    for (var i = 0; i < key_re.length; i++) {
        console.log(key_re[i]);
    }
}
